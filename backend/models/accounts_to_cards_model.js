...
const accounts_to_cards={
    ...
    getOneAccounts_To_Cards(id,callback) {
        return db.query("SELECT * FROM accounts_to_cards WHERE reference_id=?",[id],callback);
    },
    ...
    updateAccounts_To_Cards(id,updateAccounts_To_Cards,callback) {
        return db.query("UPDATE accounts_to_cards SET reference_account_id=?, reference_card_id=? WHERE reference_id=?",
        [updateAccounts_To_Cards.reference_account_id, 
            updateAccounts_To_Cards.reference_card_id,
            id],
            callback);
    },
    deleteAccounts_To_Cards(id,callback) {
        return db.query("DELETE FROM accounts_to_cards WHERE reference_id=?", [id], callback);
    }
}
